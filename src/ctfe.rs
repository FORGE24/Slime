...
//
// Adapted for Slime2 (LLVM IR backend). Part of the ETCA stack:
//   CTFE → Dynamic Precomp → TCE → IFM → DOPE → Scheduler Elim → Pre-Concurrency
//
...
//!
//! 继承自 [FORGE24/Slime](https://github.com/FORGE24/Slime)（Slime1）。
//!
//! 核心理念：
//! - 程序默认在编译期执行
//! - runtime 是退化路径，不是主战场
...
impl CtfeEngine {
    ...
    /// Clear a compile-time binding (variable may be mutated at runtime).
    pub fn clear_const(&mut self, name: &str) {
        self.const_values.remove(name);
    }

    /// Read a compile-time binding by name.
    pub fn get_const(&self, name: &str) -> Option<&CtfeValue> {
